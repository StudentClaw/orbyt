...
- Constraints and boundaries:
  - memorize reads across the student's whole workspace memory, not one chat
    thread only
...
- Acceptance criteria for this increment:
  - the memorize input surface is decision-complete
...
- end-user notification behavior for memorize results

### Acceptance Criteria

- The rollout names the thread-history inputs memorize needs from the server
  layer.
...
1. Define the memorize read surface over orchestration threads and turns:
...
- Unit:
  - thread-history readers can prove memorize sees the correct incremental turn
    window
  - course-memory merge tests can prove Canvas layout and strategy coexist in
    one course node
- Integration:
  - one memorize run can consume thread history plus a Canvas-derived course
...
- Failure path:
  - missing Canvas context must not block memorize from updating other memory
...
- the integration boundaries for memorize inputs and memory readers are
  decision-complete
